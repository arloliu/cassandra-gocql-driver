...
* The `Bind()` API provides a client app with a low level mechanism to introspect query meta data and extract appropriate field values from application level data structures. `Query.Binding()` replaces that callback on an existing query, so a reusable query can switch between callbacks or back to static values; `Bind()` and `Binding()` are mutually exclusive, and whichever you call last is the one the query uses.
